...
class MOPO():
    ...
    def rollout_transitions(self):
        ...
        for _ in range(self._rollout_length):
            ...
            # print("Output stats:", next_observations.min(), next_observations.max(), next_observations.mean())
            # print("Input stats:", observations.min(), observations.max(), observations.mean(), actions.max(), actions.min())
